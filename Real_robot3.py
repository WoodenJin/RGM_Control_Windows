...
def get_current_path(t):
    ...
    right_hip = interpolate.splev(t, tck_leg1_hip, der=0)
    right_knee = interpolate.splev(t, tck_leg1_knee, der=0)
    right_ankle = interpolate.splev(t, tck_leg1_ankle, der=0)
    left_hip = interpolate.splev(t, tck_leg2_hip, der=0)
    left_knee = interpolate.splev(t, tck_leg2_knee, der=0)
    left_ankle = interpolate.splev(t, tck_leg2_ankle, der=0)
    ...
    return np.array([right_hip, right_knee, right_ankle, left_hip, left_knee, left_ankle, upper])
...
def main():
    ...
    period = 1.5
    step = 0.01
    amp = 1* pi / 12  # unit is degree
    phase = -6 * pi / 12  # the phase between 6-dof leg motion and the torso motion
    ...
    robot = BipedRobot(init_pos, step=step, is_gui=True)
    ...
    while True:
        ...
        index += 1
        now = (index % num) / num * period
        angle_control = get_current_path(now)
        current_time = index * period / num
        # angle_control[0] = 0
        ...
        motion_data = np.vstack(
            (motion_data, np.array([current_time, pos[0], pos[1], pos[2], euler[0], euler[1], euler[2]])))
